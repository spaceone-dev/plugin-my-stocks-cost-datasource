...
class CostManager(BaseManager):
    def __init__(self, *args, **kwargs):
        ...
        self.kospi_file_connector = kospi_file_connector
    ...
    def get_data(self, options, secret_data, schema, task_options):
        ...
        response_stream = self.kospi_file_connector.get_cost_data(base_url)
    ...
    def _make_cost_data(self, results):
        ...
        for result in results:
            ...
            if self.kospi_file_connector.field_mapper:
                result = self._change_result_by_field_mapper(result)

            if self.kospi_file_connector.default_vars:
            ...
            try:
                data = {
                    ...
                    "billed_date": result.get("billed_date", ""),
                    # "billed_date": "2023-11-07",
                    "data": data,
    ...
    def _change_result_by_field_mapper(self, result):
        for (
            origin_field,
            actual_field,
        ) in self.kospi_file_connector.field_mapper.items():
    ...
    #     for key, value in self.kospi_file_connector.default_vars.items():
    #         result[key] = value
    ...
    @staticmethod
    def _make_additional_info(result):
        additional_info = {}

        if result.get("Open"):
            additional_info["Open"] = result["Open"]

        if result.get("Low"):
            additional_info["Low"] = result["Low"]

        if result.get("Close"):
            additional_info["Close"] = result["Close"]

        return additional_info

    @staticmethod
    def _make_data(result):
        data = {}
        if result.get("Change Percent", 0.0):
            data["Change Percent"] = float(result["Change Percent"])
